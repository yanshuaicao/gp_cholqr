function [UC, jitter] = jitChol(A, maxTries,form)
...
% 	jitChol.m SVN version 246
% 	last update 2009-01-24T11:05:37.000000Z

if nargin < 2
  maxTries = 10;
end
if nargin == 3 && strcmp(form,'lower')
    doLower = true;
else
    doLower = false;
end
...
%        warning(['Matrix is not positive definite in jitChol, adding ' num2str(jitter) ' jitter.'])
      ...
      if doLower
        UC = chol(real(A+jitter*eye(size(A, 1))),'lower');
      else
