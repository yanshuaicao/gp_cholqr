...
% the implementation of itive) values forces higher precision in your "f" function). The function returns the found
...
MAX = 500;                         % max 20 function evaluations per line search
RATIO = 10;                                       % maximum allowed slope ratio
SIG = .1; RHO = SIG/2; % SIG and RHO are the constants controlling the Wolfe-
...
if abs(length) <= 50
    ...
    print_epoch = ceil(abs(length)/50);
end
...
[f0 df0] = f(reshape(X,inputsize), varargin{:});          % get function value and gradient
% catch e
%     return;
% end
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        [f3 df3] = f(reshape(X+x3*s,inputsize), varargin{:});
        if isnan(f3) ||  isinf(f3) ||  any(isnan(df3)+isinf(df3)), error, end
        success = 1;
      catch  e                              % catch any error which occured in f
